...
class SalesForecastingEngine:
    ...
    def __init__(self, data_file='cleaned_customer_data.csv'):
        ...
        # NEW: Store test set metrics separately
        self.test_metrics = {}
        self.category_test_metrics = {}
    ...
    def _fit_models(self):
        """Fit ARIMA models and calculate test metrics"""
        if self.daily_sales is None or len(self.daily_sales) < 15:
            return
        
        try:
            # Main model - 80/20 train/test split
            revenue = self.daily_sales['Revenue_Smoothed'].values
            train_size = int(len(revenue) * 0.8)
            train_revenue = revenue[:train_size]
            test_revenue = revenue[train_size:]
            
            # Find best params on training data
            p, d, q = self._find_best_arima_params(train_revenue)
            
            # Train model on training data only
            train_model = EnhancedARIMAModel(p, d, q)
            train_model.fit(train_revenue)
            ...
            # Category models - 80/20 train/test split
            for category, cat_data in self.category_sales.items():
                if len(cat_data) >= 15 and cat_data['Quantity'].sum() > 0:
                    try:
                        ...
                        train_size_cat = int(len(quantity) * 0.8)
                        train_quantity = quantity[:train_size_cat]
                        test_quantity = quantity[train_size_cat:]
                        
                        # Find best params on training data
                        p, d, q = self._find_best_arima_params(train_quantity)
                        
                        # Train model on training data only
                        train_cat_model = EnhancedARIMAModel(p, d, q)
                        train_cat_model.fit(train_quantity)
                        ...
                        # Only keep models with reasonable test performance
                        if category in self.category_test_metrics:
                            test_r2 = self.category_test_metrics[category].get('r2')
                            if test_r2 is not None and test_r2 > -0.5:
    ...
    def generate_forecast(self, period='7days'):
        ...
        try:
            ...
            # Use TEST metrics for accuracy reporting
            test_r2 = self.test_metrics.get('r2', 0) if self.test_metrics else 0
            ...
            uncertainty = max(0.1, 1 - test_r2) if test_r2 > 0 else 0.5
            ...
            return {
                ...
                'modelInfo': {
                    ...
                    'accuracy': f"{round(max(0, test_r2 * 100), 1)}%",
...
def get_metrics():
    """Return ONLY test set metrics - the true measure of model performance"""
    eng = get_engine()
    if eng.arima_model is None:
        return jsonify({'error': 'No model'}), 404
    
    # If no test metrics available, return error
    if not eng.test_metrics or eng.test_metrics.get('mae') is None:
        return jsonify({'error': 'No test metrics available'}), 404
    
    # Main model test metrics - matching old format exactly
    return jsonify({
        'main_model': {
            'type': f"ARIMA({eng.arima_model.p},{eng.arima_model.d},{eng.arima_model.q})",
            'mae': round(eng.test_metrics['mae'], 2),
        ...
        'note': 'Metrics calculated on test set only (unseen 20% of data)'
    })
