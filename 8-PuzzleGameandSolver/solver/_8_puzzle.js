...
// Function to generate a random permutation of the numbers 0 to 8 that is solvable
function getRandomPermutation() {
    ...
    } while (!existSolution(numbers)); // Continue generating until we get a solvable permutation

    return numbers;
}

// Function to check if a permutation is solvable
...
// Function to randomize the puzzle with a solvable permutation
function randomize() {
    const randomNumbers = getRandomPermutation();
    displayBoard(randomNumbers);
}

// Example usage: Generating and displaying a solvable random puzzle
document.addEventListener('DOMContentLoaded', function () {
    randomize(); // Randomize and display a solvable puzzle on page load
});
